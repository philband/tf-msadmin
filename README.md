...
[![CI](https://github.com/philband/tf-msadmin/actions/workflows/ci.yml/badge.svg)](https://github.com/philband/tf-msadmin/actions/workflows/ci.yml)

Shared Terraform-Plugin-Framework runtime for the Microsoft admin providers —
[Exchange Online](https://github.com/philband/terraform-provider-exo), Purview and
Teams. Factors out the parts every one of these providers needs so each provider
repo only carries its own resources.

```bash
go get github.com/philband/tf-msadmin
```
...
resolved [`authx.Config`](https://github.com/philband/go-exoscc). The surface is
...
`LoadUntil` wraps [`consistency`](https://github.com/philband/go-msadmin): it
